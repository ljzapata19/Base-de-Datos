...
--1. Obtenga el nombre completo (first_name y last_name) y el salario (salary) de todos los empleados. 
...
--2. Obtenga el nombre completo (first_name y last_name) y el salario (salary) de todos los 
-- empleados ordenado alfabéticamente por apellido (last_name).
...
--cuya comisión sea mayor a 0.25. Mostrarlos ordenados en forma descendente por last_name
...
(select * from departments where department_id=10) union (select * from departments where department_id=70)

--6. Obtenga el apellido (last_name) de los empleados junto al nombre del departamento (department_name) 
-- donde trabajan
select last_name, department_name 
	from employees 
	join departments on employees.department_id=departments.department_id

--7. Obtenga el apellido (last_name) de los empleados de los departamentos de Finanzas (Finance) 
...
-- 8. Obtenga sin repetir los tipos de trabajos (jobs) que realizan en los departamentos los empleados. 
	-- Liste el identificador del departamento y el nombre del trabajo.
select distinct department_id, job_title 
from employees natural join jobs 

-- 9. Obtenga los departamentos en los que los empleados realizan trabajos de Contabilidad (Stock Clerk) 
--y Asistente de Administración (Shipping Clerk). Liste el identificador del departamento y 
--el nombre del trabajo.
SELECT DISTINCT d.department_id, j.job_title
FROM employees e
JOIN Jobs j ON e.job_id = j.job_id
JOIN Departments d ON e.department_id = d.department_id
WHERE j.job_title IN ('Stock Clerk', 'Shipping Clerk');
...
--11. Obtenga el apellido de los empleados (last_name) y el salario (salary) de aquellos empleados que 
--tienen un salario mayor que el salario promedio. Ordene el listado en forma descendente por salario.
...
-- 12. Obtenga el salario máximo de los empleados del departamento 110. 
...
--NOTA: En la tabla departments el atributo manager_id tiene la identificación de los 
--supervisores de cada dpto.
select *
from employees
...
--query, division resta intersecc, union, groupby having, subquery